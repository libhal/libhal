...
// TODO(#207): add note to rip this out later when we get proper scatter span

#include <array>
#include <span>

export module hal:scatter_span;
                          ...
                          scatter_span<T> const& rhs)
{
  ...
  auto lhs_outer = lhs.begin();
  auto lhs_inner = lhs_outer->begin();
  auto rhs_outer = rhs.begin();
  auto rhs_inner = rhs_outer->begin();
  ...
  while (true) {
    ...
    ++lhs_inner;
    if (lhs_inner == lhs_outer->end()) {
      ++lhs_outer;
      if (lhs_outer == lhs.end()) {
        ++rhs_inner;
        if (rhs_inner == rhs_outer->end()) {
          ++rhs_outer;
          return rhs_outer == rhs.end();
        }
        return false;
      }
      lhs_inner = lhs_outer->begin();
    }

    ++rhs_inner;
    if (rhs_inner == rhs_outer->end()) {
      ++rhs_outer;
      if (rhs_outer == rhs.end()) {
        return false;
      }
      rhs_inner = rhs_outer->begin();
    }
  }
}
...
namespace hal {
export using v5::make_scatter_array;
export using v5::make_scatter_bytes;
export using v5::make_writable_scatter_bytes;
export using v5::scatter_span;
export using v5::spanable;
export using v5::spanable_bytes;
export using v5::spanable_writable_bytes;
export using v5::operator==;
export using v5::operator!=;
}  // namespace hal
