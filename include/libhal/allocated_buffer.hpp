...
public:
   ...
   */
  explicit allocated_buffer(std::pmr::polymorphic_allocator<byte> p_allocator,
                            size_type p_size)
   ...
   * @throws std::bad_alloc if memory allocation fails
   */
  allocated_buffer(std::pmr::polymorphic_allocator<byte> p_allocator,
                   size_type p_size,
                   T const& p_value)
    : m_size(std::max<size_type>(1, p_size))
    , m_allocator(p_allocator)
  ...
  {
    if (p_index >= m_size) {
      throw std::out_of_range("allocated_buffer::at(idx)");
    }
