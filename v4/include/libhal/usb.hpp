...
struct setup_packet
{
   ...
   */
  constexpr setup_packet(bool p_device_to_host, args p_args)
  {

    raw_request_bytes[0] = p_device_to_host << 7 |
                           static_cast<byte>(p_args.type) << 5 |
                           static_cast<byte>(p_args.recipient);
    raw_request_bytes[1] = p_args.request;

    constexpr usize value_offset = 2;
    set_le_u16<value_offset>(p_args.value);

    constexpr usize index_offset = 4;
    set_le_u16<index_offset>(p_args.value);

    constexpr usize length_offset = 6;
  ...
  {
    constexpr usize value_offset = 4;
    set_le_u16<value_offset>(p_index);
  }

  constexpr void length(u16 p_length)
  {
    constexpr usize value_offset = 6;
    set_le_u16<value_offset>(p_length);
  }
